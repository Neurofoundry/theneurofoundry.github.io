...
import anvilImg from 'figma:asset/73a3f4122e938ec77473f228960ed7e9bb473041.png';
import firePillarImg from 'figma:asset/076c83a66a3878bf5497c7da808dc6137b2ba457.png';
import forgeDetailImg from 'figma:asset/fed600b5afdcc2e064f7813737149ffcdad477ac.png';
import forgeFrameImg from 'figma:asset/cedd0340394ba615c5c8cfcd16b72d0a823fa7f3.png';
import forgeBgImg from 'figma:asset/ff3935e86b0b343bba746b512a8caa450b5864b6.png';
...
export function ForgePage() {
  ...
  const handleOriginUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleFusionUpload = useCallback((index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (e) => {
        const newFusions = [...fusionImages];
        newFusions[index] = e.target?.result as string;
        setFusionImages(newFusions);
      };
      reader.readAsDataURL(file);
    }
  }, [fusionImages]);
  ...
  const canGenerate = originImage && fusionImages.some(img => img !== null) && prompt.trim();
  ...
  return (
    <div className="h-full bg-black relative overflow-hidden">
      ...
      <div 
        ...
        style={{
          backgroundImage: `url(${forgeBgImg})`,
      ...
      {/* Main content */}
      <div className="relative z-35 h-full flex items-center justify-center p-8">
        {/* Central media viewer inside forge opening */}
        <div 
          className="absolute left-1/2 top-[52%] -translate-x-1/2 -translate-y-1/2 z-20"
          style={{
            width: '42%',
            aspectRatio: '16 / 10',
          }}
            ...
            >
              ...
              {originImage ? (
                <>
                  ...
                  <button
                    onClick={(e) => { e.preventDefault(); setOriginImage(null); }}
              ...
              {fusionImages.map((img, index) => (
                ...
                >
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleFusionUpload(index, e)}
                    className="hidden"
                  />
            ...
            <div className="mt-3 flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-black/50 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ 
                    width: `${(fusionImages.filter(img => img !== null).length / 3) * 100}%`,
                    boxShadow: '0 0 10px rgba(255, 107, 61, 0.6)'
                  }}
                />
              </div>
              <span className="text-[9px] text-gray-500 font-medium">
                {fusionImages.filter(img => img !== null).length}/3
              </span>
            </div>
          </div>
      ...
      <style>{`
        ...
        @keyframes sparkFly {
          ...
          100% {
            ...
            transform: 
              translate(
                calc(cos(var(--spark-angle) * 3.14159 / 180) * var(--spark-distance)),
                calc(sin(var(--spark-angle) * 3.14159 / 180) * var(--spark-distance))
              )
