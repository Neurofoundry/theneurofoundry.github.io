...
function compileStyledPrompt(input: {
  ...
  const promptPlain = style.slot_order
    .map((slot) => (ordered[slot] || []).join(", "))
    .filter(Boolean)
    .join(", ");

  const negativeUser = String(input.negative_prompt || "").trim();
  const negative = [negativeUser, ...style.negative_defaults].filter(Boolean).join(", ");
...
async function analyzeMode(env: Env, mode: Mode, imageBase64: string): Promise<{
  model: string;
  latency_ms: number;
  attempts: number;
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/style/compile") {
      ...
      const compiled = compileStyledPrompt({
        ...
        cdp: body.cdp as Record<string, unknown> | string | undefined,
        acp: body.acp as Record<string, unknown> | string | undefined,
        sip: body.sip as Record<string, unknown> | string | undefined
      });
      return jsonResponse(200, { ok: true, ...compiled }, corsHeaders);
    }
