...
function splitFragments(text, maxParts = 8) {
  return text.split(/[.,;|\n]/g).map((s) => s.trim().toLowerCase()).filter(Boolean).slice(0, maxParts).map((s) => s.replace(/\s+/g, " "));
}
__name(splitFragments, "splitFragments");
function pickStyleProfile(styleId) {
  return STYLE_PROFILES.find((s) => s.id === styleId) || STYLE_PROFILES[0];
}
__name(pickStyleProfile, "pickStyleProfile");
function wordCount(text) {
  return text.trim() ? text.trim().split(/\s+/).length : 0;
}
__name(wordCount, "wordCount");
function compactToBudget(fragments, maxWords) {
  const out = [];
  let used = 0;
...
__name(compactToBudget, "compactToBudget");
function bracket(slotValues) {
  return `[${slotValues.join(", ")}]`;
}
__name(bracket, "bracket");
function compileStyledPrompt(input) {
  const style = input.style;
  const cdp = sanitizeAnalysis("cdp", typeof input.cdp === "object" && input.cdp ? input.cdp : null, String(input.cdp || ""));
  const acp = sanitizeAnalysis("acp", typeof input.acp === "object" && input.acp ? input.acp : null, String(input.acp || ""));
  const sip = sanitizeAnalysis("sip", typeof input.sip === "object" && input.sip ? input.sip : null, String(input.sip || ""));
  ...
  const promptCompact = style.slot_order.map((slot) => bracket(ordered[slot] || [])).join(" ").replace(/\[\]/g, "").replace(/\s+/g, " ").trim();
  const promptPlain = style.slot_order.map((slot) => (ordered[slot] || []).join(", ")).filter(Boolean).join(", ");
...
__name(compileStyledPrompt, "compileStyledPrompt");
...
// .wrangler/tmp/bundle-MUV9Po/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-MUV9Po/middleware-loader.entry.ts
